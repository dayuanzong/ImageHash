...
uses
  ...
  Buttons, EditBtn, uNotifier, uThreadHashing, uThreadClassifier, uFrmPathEditor, Types;
...
type
  ...
  private
    ...
    fRunStarted: QWord;
    fSummaryLogIndex: Integer;
    fLastClusterCount: Integer;
    fLastMultiClusterCount: Integer;
    fLastSummaryUpdate: QWord;
    procedure FreeData;
    ...
    function FormatDurationMs(ms: QWord): string;
    procedure UpdateSummaryLog(elapsedMs: QWord; clusterCount, totalCount, multiCount: Integer);
...
uses
  Math, IntfGraphics, FPimage, lcltype, LCLIntf, fpcanvas,
...
begin
  ...
  for i:= 0 to ImageHandlers.Count-1 do begin
    ...
    if tn = 'JPEG Graphics' then tn := 'JPEG 图像'
    else if tn = 'Portable Network Graphic' then tn := 'PNG 图像'
    else if tn = 'Bitmap Image' then tn := '位图图像'
    else if tn = 'Targa Image File' then tn := 'Targa 图像'
    else if tn = 'GIF Image' then tn := 'GIF 图像'
    else if tn = 'TIFF Image' then tn := 'TIFF 图像';

    clbFilter.Items.Add('%s (%s)', [tn, ex]);
    if not Assigned(ImageHandlers.ImageReader[ImageHandlers.TypeNames[i]]) then
...
begin
  ...
  try
    ...
    fLastMultiClusterCount:= 0;
    fLastSummaryUpdate:= 0;
    UpdateSummaryLog(0, 0, Length(fImageInfos), 0);

    lbStatus.Caption:= '正在设置分类器...';
    RunClassifier;
    btnRecompare.Enabled:= true;

    lbStatus.Caption:= '正在设置加载器...';
    ...
    if loadermemerror then begin
      meLog.Lines.Add('- 遇到内存不足，正在重新运行失败的单线程任务');
    ...
    UpdateSummaryLog(t2-t1, fClassifier.Clusters.Count, Length(fImageInfos), k);
    {$IfDef DEBUG}
    PrintMemStats;
    {$EndIf}
  finally
...
function TfmMain.FormatDurationMs(ms: QWord): string;
const
  SecMs = 1000.0;
  MinMs = 60.0 * SecMs;
  HourMs = 60.0 * MinMs;
...
begin
  if ms < 1000 then
    Exit(Format('%d毫秒', [ms]));
  if ms < Round(MinMs) then
    Exit(Format('%.1f秒', [ms / SecMs]));
  if ms < Round(HourMs) then
    Exit(Format('%.1f分钟', [ms / MinMs]));
  if ms < Round(DayMs) then
    Exit(Format('%.1f小时', [ms / HourMs]));
  Result:= Format('%.1f天', [ms / DayMs]);
end;

procedure TfmMain.UpdateSummaryLog(elapsedMs: QWord; clusterCount, totalCount, multiCount: Integer);
var
  s: string;
begin
  s:= Format('耗时: %s | 分组: %d / %d | 分组>1: %d',
    [FormatDurationMs(elapsedMs), clusterCount, totalCount, multiCount]);
  if fSummaryLogIndex < 0 then
    fSummaryLogIndex:= meLog.Lines.Add(s)
  else
    meLog.Lines[fSummaryLogIndex]:= s;
end;
...
procedure TfmMain.NotifyProgress;
  ...
  begin
    ...
    s:= Format('%d/%d, %.1f%%'+sLineBreak+'已用时间: %s', [pb.Position, pb.Max, progress * 100, FormatTime(trunc(elapsed))]);
    if total > 24*3600 then
      pb.Hint:= s
    else
      pb.Hint:= s + sLineBreak + Format('剩余时间: %s', [FormatTime(Ceil(total - elapsed))]);
  end;
...
begin                      
  ...
  if pbClassifier.Position <> fNotifier.Classified then begin
    ...
    try
      ...
      fLastClusterCount:= fClassifier.Clusters.Count;
      fLastMultiClusterCount:= clusters.Count;
  ...
  if fRunStarted <> 0 then begin
    elapsedMs:= GetTickCount64 - fRunStarted;
    if (fLastSummaryUpdate = 0) or (elapsedMs - fLastSummaryUpdate >= 1000) then begin
      fLastSummaryUpdate:= elapsedMs;
      UpdateSummaryLog(elapsedMs, fLastClusterCount, Length(fImageInfos), fLastMultiClusterCount);
    end;
  end;
end;
...
begin
  ...
  meLog.Lines.Add('总使用 = %d k', [szAll shr 10]);
  meLog.Lines.Add('列表使用 = %d k', [szList shr 10]);
  meLog.Lines.Add('分组使用 = %d k', [szClusters shr 10]);
  hs:= GetFPCHeapStatus;
  meLog.Lines.Add('最大堆大小 = %d k', [hs.MaxHeapSize shr 10]);
  meLog.Lines.Add('最大堆使用 = %d k', [hs.MaxHeapUsed shr 10]);
  meLog.Lines.Add('当前堆大小 = %d k', [hs.CurrHeapSize shr 10]);
  meLog.Lines.Add('当前堆使用 = %d k', [hs.CurrHeapUsed shr 10]);
end;
...
var
  ...
  c: TCluster;
  im: PImageInfoItem;
  procedure OpenFolderAndSelectFile(const fn: string);
  begin
    OpenDocument(ExtractFileDir(fn));
  end;
begin
...
begin
  ...
  try
    ...
    if marked.Count = 0 then begin
      MessageDlg('未选择要删除的项目！', mtInformation, [mbOK], 0);
      exit;
    end;
    if MessageDlg(Format('已选择 %d 个项目进行删除，是否继续？', [marked.Count]), mtInformation, mbYesNo, 0) <> mrYes then
      exit;
...
begin
  MessageDlg('选择要删除或保留的图片。' + sLineBreak +
             '左键: 切换删除/保留' + sLineBreak +
             '右键: 切换忽略/取消忽略' + sLineBreak +
             '中键, Ctrl+左键: 在默认应用中打开图片' + sLineBreak +
             'Shift+中键, Ctrl+Shift+左键: 在所在文件夹中显示',
...
begin
  MessageDlg(Application.Title + sLineBreak + 'https://github.com/martok/ImageHash',
             mtInformation, [mbOK], 0);
end;
